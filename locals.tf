locals {
  ...
  customer_managed_policies = flatten([
    for permission in var.permissions_list : [
      for policy in permission.customer_managed_policies : {
        permission_set_name = permission.name
  ...
  customer_managed_policy_arns = {
    for policy in local.customer_managed_policies :
    "${policy.permission_set_name}.${policy.policy_name}.${policy.policy_path}" => policy
  }
